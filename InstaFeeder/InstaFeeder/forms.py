...
from InstaFeeder.models import User, Photo,PhotoLikes,Followers,PhotoTag
...
class AjaxPhotoFeed(Ajax):
    def validate(self):
        try:
            ...
            for user in User.objects.filter(username_in=followerslist):
                profilepics[user.username]=user.profilepic
                if user.profilepic=="":
                    profilepics[user.username]="stati/assets/img/default.png"


            # for item in Photo.objects.filter(owner=self.user.username).order_by('-date_uploaded')[int(self.start):int(self.start)+3]:

            for item in Photo.objects.filter(owner_in=followerslist).order_by('-date_uploaded')[int(self.start):int(self.start)+3]:
...
class AjaxFollow(Ajax):
    def validate(self):
        ...
        if self.user.username == self.follower:
            return self.error("Can't follow Yurself")

        if not Followers.objects.filter(liker=self.follower,follower=self.user.username).exists():
